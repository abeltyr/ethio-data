...
import { getDb, closeDatabase, dbPath, REPO_ROOT } from "@ethiodata/database";
import { DOMAINS, type Domain } from "@ethiodata/types";
...
// dbPath() is an absolute, repo-root-anchored path (for file IO); docs display the stable
// repo-relative path instead.
const rel = (d: Domain) => `data/db/${d}.db`;
...
function writeDomainDoc(domain: Domain): { tables: number; rows: number; span: string } {
  ...
  w(`\n_Auto-generated ${now} UTC by \`src/report.ts\`. File: \`${rel(domain)}\`._`);
  ...
  w(`\n## Querying\n`);
  w("```bash");
  w(`sqlite3 ${rel(domain)} "SELECT * FROM ${info.fact} LIMIT 5;"`);
  w("```");
  if (domain === "prices") {
    w(`\nCPI-deflated (real) prices need the macro DB — attach it:`);
    w("```sql");
    w(`ATTACH '${rel("macro")}' AS macro;`);
...
function writeIndex(summary: Record<Domain, { tables: number; rows: number; span: string }>): void {
  ...
  w(mdTable(["domain DB", "tables", "rows", "span", "doc"],
    DOMAINS.map(d => [`\`${rel(d)}\``, summary[d].tables, n(summary[d].rows), summary[d].span, `\`data/db/${d}.md\``])));
  ...
  writeFileSync(join(REPO_ROOT, "DATA_PROFILE.md"), out.join("\n") + "\n");
