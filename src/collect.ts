import { getProviderDb, closeDatabase } from "./shared/database";
import { isFresh, recordFetch, forceRefetch } from "./shared/database/ledger";
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    const db = getProviderDb(step.id);
